...
inductive Kernel where
  ...
  -- cols: number of input columns per row.
  -- program: E-graph optimized, let-lifted scalar function (typed LowLevelProgram).
  -- Generated by lower(MatExpr.mapScalarExpr) which has access to the Expr.
  | mapScalar : (cols : Nat) → (program : AmoLean.LowLevelProgram) → Kernel
  /-- Z → GF(2)^width: decompose an integer into its binary representation.
      Input: 1 element (integer). Output: width elements (0 or 1).
      Constraint: d = Σ bᵢ·2ⁱ, each bᵢ ∈ {0,1}. -/
  | bitDecompose : (width : Nat) → Kernel
  /-- GF(2)^width → Z: compose binary bits into an integer.
      Input: width elements (0 or 1). Output: 1 element (integer).
      Computes: d = Σ bᵢ·2ⁱ. -/
  | bitCompose : (width : Nat) → Kernel
  deriving Repr, BEq, Inhabited
...
def inputSize : Kernel → Nat
  ...
  | addRoundConst _ n => n
  | butterfly4 => 4
  | mapScalar cols _ => cols
  | bitDecompose _ => 1
  | bitCompose width => width
...
def toString : Kernel → String
  ...
  | mapScalar cols prog => s!"MapScalar({cols}, {prog.assignments.length} assigns)"
  | bitDecompose width => s!"BitDecompose({width})"
  | bitCompose width => s!"BitCompose({width})"
...
def lower (m n : Nat) (state : LowerState) (mExpr : MatExpr α m n) : (SigmaExpr × LowerState) :=
  ...
  | @MatExpr.bitDecompose _ rows width inputMat =>
    -- Lower inner (n×1), then for each row: decompose the single value into `width` bits
    let (innerExpr, state1) := lower rows 1 state inputMat
    let (loopVar, state2) := freshLoopVar state1
    let rowGather := Gather.contiguous 1 (.var loopVar)
    let rowScatter : Scatter := { count := width, baseAddr := .affine 0 width loopVar, stride := 1 }
    let body := .compute (.bitDecompose width) rowGather rowScatter
    (.seq innerExpr (.loop rows loopVar body), state2)

  | @MatExpr.bitCompose _ rows width inputMat =>
    -- Lower inner (n×width), then for each row: compose `width` bits into a single value
    let (innerExpr, state1) := lower rows width state inputMat
    let (loopVar, state2) := freshLoopVar state1
    let rowGather : Gather := { count := width, baseAddr := .affine 0 width loopVar, stride := 1 }
    let rowScatter := Scatter.contiguous 1 (.var loopVar)
    let body := .compute (.bitCompose width) rowGather rowScatter
    (.seq innerExpr (.loop rows loopVar body), state2)
