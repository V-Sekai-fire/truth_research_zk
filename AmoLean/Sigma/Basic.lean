...
inductive Kernel where
  ...
  -- scalarBody: pre-expanded scalar assignments (from Expr → toLowLevel → ScalarBlock).
  -- Generated by lower(MatExpr.mapScalarExpr ...) which has access to the Expr.
  | mapScalar : (cols : Nat) → (scalarBody : List (String × String)) → Kernel
  deriving Repr, BEq, Inhabited
...
def toString : Kernel → String
  ...
  | mapScalar cols body => s!"MapScalar({cols}, {body.length} assigns)"
...
def lower (m n : Nat) (state : LowerState) (mExpr : MatExpr α m n) : (SigmaExpr × LowerState) :=
  ...
  | @MatExpr.mapScalarExpr _ rows cols scalarFn inputMat =>
    -- Lower the input matrix, then loop over rows applying the scalar kernel.
    -- Convert Expr Int → LowLevelProgram → (varName, valueString) pairs.
    ...
    -- E-graph optimize the scalar polynomial, then lower to let-lifted SSA
    let optExpr := (AmoLean.EGraph.optimizeBasic scalarFn).getD scalarFn
    let varNames : AmoLean.VarId → String := fun i => s!"x{i}"
    let lowered := AmoLean.toLowLevel varNames optExpr
    -- Convert LowLevelProgram assignments to (name, value) string pairs
    -- Convert LowLevelExpr to string (inline since no toString available)
    let rec llStr : AmoLean.LowLevelExpr → String
      | .litInt n => s!"{n}"
      | .varRef s => s
      | .binOp op l r => s!"({llStr l} {op} {llStr r})"
      | .funcCall f args => s!"{f}({String.intercalate ", " (args.map llStr)})"
    let bodyPairs := lowered.assignments.map fun a =>
      (a.varName, llStr a.value)
    let body := .compute (.mapScalar cols bodyPairs) rowGather rowScatter
